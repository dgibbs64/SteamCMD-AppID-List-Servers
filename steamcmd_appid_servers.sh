...
cat steamcmd_appid_servers.json | jq '.applist.apps[]' | jq -r '[.appid, .name] | @csv' > steamcmd_appid_servers.csv
echo "Creating steamcmd_appid_servers.md"
cat steamcmd_appid_servers.json | jq '.applist[]' | md-table > steamcmd_appid_servers.md
echo "exit"
exit
