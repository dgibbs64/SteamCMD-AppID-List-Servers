...
steam_servers=$(cat steamcmd_getapplist.json | jq '[.applist.apps[] | select(.name | contains("server","Server")) |  jq '[.applist.apps[] | select(.name | contains("1054680")|not)| jq -s '.[]|sort_by(.appid)')
